...
psf_fin=np.zeros((len(freq),11,11))
psf_oa=np.zeros((len(freq),11,11))
scaling_factor=np.zeros((len(freq)))
fwhm=[]
it=0
...
for i in range(0,psf.shape[0]):
    ...
    fit = vip.var.fit_2dgaussian(psfn, crop=True, cropsize=9, debug=False)
    fwhm.append(float((fit.fwhm_y+fit.fwhm_x)/2))
    psfn = vip.preproc.frame_crop(psfn, 19)   
    psfn = vip.fm.normalize_psf(psfn, float((fit.fwhm_y+fit.fwhm_x)/2))
    psf_fin[it,:,:]=  vip.preproc.frame_crop(psfn,11)
    psf_oa[it,:,:]=vip.preproc.frame_crop(psf,11)*scal_fact
    
    indices = get_annulus_segments(psf_fin[it],0,round((fwhm)/2),1) 
    indices_oa = get_annulus_segments(psf_oa[it],0,round((fwhm)/2),1) 
    scaling_factor[it]=psf_oa[it,indices_oa[0][0], indices_oa[0][1]].sum()/psf_fin[it,indices[0][0], indices[0][1]].sum()
...
#Computation of a RSM detection map using ADI + SDI data (very slow, if possible reduce the number of channels)
      
f=PyRSM(np.mean(fwhm),minradius=min_radius,maxradius=max_radius,pxscale=pxscale,ncore=ncore,inv_ang=True,trunc=10)

f.add_cube(psf_fin,cube, angs,freq)
...
f.add_method('LLSG',interval=[5],crop_size=3,crop_range=1,asize=5,opti_bound=[[1,5],[1,4]])
...
f.opti_combination(estimator='Forward',colmode='median',threshold=False,contrast_sel='Max',combination='Bottom-Up') 
f.save_parameters('/.../','Opti_param')
...
flux=np.zeros((len(freq),pscy))
centroid_y=np.zeros((len(freq),pscy))
centroid_x=np.zeros((len(freq),pscy))
rad_dist=np.zeros((len(freq),pscy))
theta=np.zeros((len(freq),pscy))
best_cost=np.zeros((len(freq),pscy))
err_phot=np.zeros((len(freq),pscy))
erry=np.zeros((len(freq),pscy))
errx=np.zeros((len(freq),pscy))
err_phot95=np.zeros((len(freq),pscy))
erry95=np.zeros((len(freq),pscy))
errx95=np.zeros((len(freq),pscy))
...
if len(pscy)>4:
    ...
    if len(pscy)>0:
        for k in range(len(pscy)):
            for i in range(len(freq)):
                ...
                #Computation of a channel-wise RSM detection map
    
                f=PyRSM(fwhm[i],minradius=min_radius,maxradius=max_radius,pxscale=pxscale,ncore=ncore,inv_ang=True,trunc=10)
                
                f.add_cube(psf_fin[i],cube[i], angs)
                ...
                f.opti_RSM(estimator='Forward',colmode='median')
                f.opti_combination(estimator='Forward',colmode='median',threshold=False,contrast_sel='Max',combination='Bottom-Up') 
                #f.save_parameters('/.../','Opti_param_channel_'+str(i))
                
                f.opti_map(estimator='Forward',colmode='median',threshold=False)
                plot_frames(f.final_map)
                opti_map=np.copy(f.final_map)
                
                #os.chdir('/.../')
                #vip.fits.write_fits('Opti_map_channel_'+str(i), f.final_map, header=None, precision=np.float32,
                        #   verbose=True)
                        
            
                #Definition of the initial photometry via the computation of a S/N based contrast curve
            
                indices = get_annulus_segments(psf_fin[i],0,round((fwhm)/2),1) 
                indices_oa = get_annulus_segments(psf_oa[i],0,round((fwhm)/2),1) 
                scaling_factor=psf_oa[i][indices_oa[0][0], indices_oa[0][1]].sum()/psf_fin[i][indices[0][0], indices[0][1]].sum()
                indc = disk(((pscy[k],pscx[k])),5)
                posy_fc, posx_fc=np.where(opti_map==np.max(opti_map[indc[0],indc[1]]))
                
                ini_cc = contrast_curve(cube[i], angs, psf_fin[i], fwhm[i], pxscale,
                ...
                cube_empty[i] = cube_inject_companions(cube[i], psf_fin[i], angs, flevel=-flux[k]*scaling_factor[i], plsc=pxscale, 
    ...
    an_dist, comp_curve = vip.metrics.completeness_curve(cube_empty[0], angs, psfn, fwhm[0], vip.psfsub.pca_annular, an_dist=an_dist,
